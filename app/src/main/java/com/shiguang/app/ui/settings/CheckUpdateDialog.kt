    ...
    LaunchedEffect(attempt) {
        ...
        when (val result = UpdateClient.fetchLatest()) {
            ...
            is FetchResult.Release -> {
                if (!AppVersion.hasUpdate(local, result.info.versionName)) {
                    state = UpdateUiState.Latest
                } else if (result.info.apkUrl == null) {
                    state = UpdateUiState.Failed("新版本缺少 APK 资产，无法下载")
                } else {
                    state = UpdateUiState.Found(result.info)
                }
            }
        }
    }
        ...
        text = {
            Column(
                ...
                when (val s = state) {
                    ...
                    is UpdateUiState.NoRelease -> {
                        Text(
                            text = "GitHub 仓库还没有发布过版本\n发布流程见 README（tag v 版本号 + 上传 APK 资产）",
                            style = MaterialTheme.typography.bodyMedium,
