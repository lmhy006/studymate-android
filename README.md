...
| 2. 习惯打卡 | ✅ | 每日 / 每周 / 自定义每 N 天；每周期最多打卡 1 次；**月历打卡视图**、**连续打卡统计**（当前/最长/累计）、**断签提示**（APP 内横幅 + 列表红字） |
| 3. 日历日程 | ✅ | 周视图（课表式，参照 BIT101-Android 课表）+ 月视图（月历 + 当日议程）；单次日程 + **批量周期日程**（例：3月1日~6月30日之间每个周一上午 08:00~09:50，一次定义，自动生效） |
...
| 6. 设置与更新 | ✅ | 亮/暗/跟随系统主题切换；关于（版本信息）；**应用内检查更新**：读取 GitHub 最新 Release，有新版时下载 APK 并调用系统安装器更新 |
...
**打卡**
- [ ] 新建习惯，选每日/每周/每 N 天；今天到期时出现“打卡”按钮，点按后变“已打卡”
- [ ] 再点“已打卡”可撤销；月历中已打卡日有实心点、断签日有红点、今天有圆圈
- [ ] 展开卡片看到“连续 X 天 / 最长 X 天 / 累计 X 次”；漏打后出现“已断签 N 次”红字与顶部横幅
- [ ] 日历内可点今日/过去日期补签；未来周期日不可提前打卡
