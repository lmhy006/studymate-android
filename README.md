...
| 3. 日历日程 | ✅ | **周视图（BIT101 式节次课表）**：13 节（08:00 起，**时间表可在 设置→日程设置 自定义每节上课/下课**），节次分界线/边框/今日高亮可开关；月视图；单次日程 + **批量周期日程**；**导入 BIT101 课表 JSON / 逐课速录（教学周起点对齐）**；**DDL（课程作业截止）**：可选关联课程、截止日期+时刻，完成后 N 天自动释放，课程序块联动 |
...
**日程设置**
- [ ] 设置 → 日程设置 → 时间表：逐节修改上课/下课时间，保存后周视图与课表导入即时按新时间表换算；“恢复默认”
- [ ] 设置 → 日程设置 → 本学期教学周起点：设成开学第一周日期后，逐课速录与 JSON 导入的“第1周”与该周对齐
...
- `weekday`：1=周一 … 7=周日；`start_section/end_section`：1..20 节（时间按当前时间表换算，默认 08:00–20:55）；
- `weeks`：教学周数组，导入时取首尾周作为日期范围（非连续周次按首尾覆盖，见“已知限制”）；
- `week_parity`（可选）：单双周，`"odd"`/`"single"`/`"单周"` 为单周，`"even"`/`"double"`/`"双周"` 为双周，缺省每周；
- 导入需选择“学期开始日期”（默认读「教学周起点」设置），每条课程将生成一条周期日程，备注带教师·周次·单双周；
